...
public class TelaLeitoresCadastro extends javax.swing.JInternalFrame {
    ...
    private String tratarDocumento(String str){
        return Normalizer.normalize(str, Normalizer.Form.NFD).replaceAll("[^\\p{ASCII}]-./*+1234567890();:?^~´'`", ""); 
    }
    ...
    private void btnSalvarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSalvarActionPerformed
        ...
        String nome = tratarDocumento(txtNome.getText());
        String dataNascimento = txtDataNascimento.getText();        
        String cpf = tratarDocumento(txtCPF.getText());
        String rg = tratarDocumento(txtRG.getText());
        String sexo = cbSexo.getSelectedItem().toString();
        String cep = tratarDocumento(txtCEP.getText());
        String bairro = txtBairro.getText();
        String complemento = txtComplemento.getText();
        String endereco = txtEndereco.getText();
        String numero = txtNumero.getText();
        String estado = cbbEstado.getSelectedItem().toString();
        String telefone = tratarDocumento(txtTelefone.getText());
        
        if(validarData(dataNascimento) == true &&
                validarCPF(cpf) == true){
            System.out.println("Entrando para cadastro");
            
            if(userLogado == null) System.out.println("Vazio usuario Logado");
            
            //realizando o cadastro no banco de dados
            CadastroDados cadastro = new CadastroDados();
            if(cadastro.cadastroLeitor(userLogado.getId_usuario(), nome, dataNascimento, 
                    cpf, rg, sexo, cep, bairro, complemento, 
                    endereco, numero, estado, telefone)){
                System.out.println("Cadastrou");
                JOptionPane.showMessageDialog(null, "CADASTRO REALIZADO COM SUCESSO!", "Informativo", HEIGHT);
            }else{
                JOptionPane.showMessageDialog(null, "CADASTRO NÃO REALIZADO, VERIFIQUE OS DADOS!", "Informativo", HEIGHT);
            }
        }
